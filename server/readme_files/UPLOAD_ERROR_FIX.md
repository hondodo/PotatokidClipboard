...
### 错误原因
1. **HTTP请求体格式不兼容**: `http.post()` 无法处理 `html.FormData`
2. **数据类型序列化失败**: FormData对象无法被正确序列化
3. **请求库兼容性问题**: 不同HTTP库对FormData的处理方式不同
      ...
      if (request.status == 200) {
        try {
          ...
          if (response['success'] == true) {
            ...
            _showError('上传失败: ${response['message'] ?? '未知错误'}');
          }
        } catch (e) {
